...
namespace esphome {
namespace m5stack_unit_acmeasure {

static const char *const TAG = "m5stack_unit_acmeasure";

void UnitACMeasureComponent::setup() {
  ESP_LOGCONFIG(TAG, "Setting up Unit ACMeasure...");

  uint8_t fw = 0;
  if (!this->read_register_(FIRMWARE_VERSION_REG, &fw, 1)) {
...
void UnitACMeasureComponent::update() {
  ...
  uint8_t ready = 0;
  if (!this->read_register_(UNIT_ACMEASURE_GET_READY_REG, &ready, 1) || ready == 0) {
    ESP_LOGW(TAG, "Measurement data not ready");
    return;
  }
  ...
  if (this->power_factor_sensor_ != nullptr) {
    uint8_t raw = 0;
    if (this->read_register_(UNIT_ACMEASURE_POWER_FACTOR_REG, &raw, 1)) {
...
bool UnitACMeasureComponent::read_register_(uint8_t reg, uint8_t *data, uint8_t len) {
  if (len == 0) {
    // Only write register address to test bus
    return this->write(&reg, 1) == i2c::ERROR_OK;
  }
  ...
  if (this->read(data, len) != i2c::ERROR_OK) {
    ESP_LOGW(TAG, "Failed to read %u bytes from register 0x%02X", len, reg);
    return false;
  }
...
bool UnitACMeasureComponent::read_u16_(uint8_t reg, uint16_t *value) {
  uint8_t buf[2];
  if (!this->read_register_(reg, buf, sizeof(buf))) {
    return false;
  }
